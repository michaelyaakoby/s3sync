        ...
        return common.queryAgentByUserUidAndSubnet(event.userUuid, subnet)
            .then(function (agent) {
                if (agent.Count == 1) {
                    ...
                    var command = '/opt/NetApp/s3sync/agent/scripts/invoke-zapi.py --address ' + event.ip + ' --user ' + event.clusterUsername + ' --password ' + event.clusterPassword + ' --sns-topic ' + common.sns_topic + ' --request ' + request + ' --request-id ' + event.requestId;
